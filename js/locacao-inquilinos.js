// PS Corretor de Imóveis — Inquilinos (Locação)
document.addEventListener('DOMContentLoaded', () => {
  ...
  const fields = ['nome', 'cpfCnpj', 'telefone', 'email', 'endereco', 'observacoes'];
  const dbColumn = { cpfCnpj: 'cpf_cnpj' };

  function openModal(record) {
    form.reset();
    formError.hidden = true;
    document.getElementById('inquilinoId').value = record ? record.id : '';
    modalTitle.textContent = record ? 'Editar inquilino' : 'Novo inquilino';
    fields.forEach(f => {
  ...
  document.getElementById('btnNovoInquilino').addEventListener('click', () => openModal(null));
    ...
    if (!data.length) {
      tableWrap.innerHTML = `
        <div class="empty-state">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><circle cx="12" cy="8" r="4"/><path d="M4 21c0-4.4 3.6-7 8-7s8 2.6 8 7"/></svg>
  ...
  tableWrap.addEventListener('click', async (e) => {
    ...
    const record = (window.__inquilinos || []).find(p => p.id === id);

    if (btn.dataset.action === 'edit') {
      openModal(record);
    } else if (btn.dataset.action === 'delete') {
      if (!confirm(`Excluir o inquilino "${record.nome}"? Essa ação não pode ser desfeita.`)) return;
      const { error } = await supabaseClient.from('inquilinos').delete().eq('id', id);
      if (error) {
        alert('Não foi possível excluir: ' + error.message + (error.message.includes('foreign key') ? '\n\nEsse inquilino possui contratos vinculados.' : ''));
        return;
      }
      loadInquilinos();
    }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const query = id
      ? supabaseClient.from('inquilinos').update(payload).eq('id', id)
      : supabaseClient.from('inquilinos').insert(payload);
